...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Branch: {{ $branch->branchName }}</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <form action="{{ route('branches.update', $branch) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="branchName" name="branchName" value="{{ old('branchName', $branch->branchName) }}" required>
                </div>
                <div class="mb-6">
                    <label for="judge_name" class="block text-gray-700 text-sm font-bold mb-2">Judge Name (Optional):</label>
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="judge_name" name="judge_name" value="{{ old('judge_name', $branch->judge_name) }}">
                </div>
                <div class="flex items-center justify-between">
                    <button type="submit" class="px-6 py-2.5 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center gap-2">
                        ...
                        <span class="font-medium">Update Branch</span>
                    </button>
                </div>
            </form>
        </div>
